...
begin
  case cap of
    ...
    GL_DEPTH_TEST: d3dDevice.SetRenderState( D3DRS_ZENABLE, LongWord(D3DZB_TRUE) );
    GL_SCISSOR_TEST:
      begin
        {$IFDEF USE_DIRECT3D8}
        ScissorEnabled := TRUE;
...
begin
  case cap of
    ...
    GL_DEPTH_TEST: d3dDevice.SetRenderState( D3DRS_ZENABLE, LongWord(D3DZB_FALSE) );
    GL_SCISSOR_TEST:
      begin
        {$IFDEF USE_DIRECT3D8}
...
begin
  ...
  d3dDevice.MultiplyTransform( TD3DTransformStateType(d3dMatrixMode), d3dMatrices[ LongWord( d3dMatrixMode ) ] );
end;
...
begin
  case factor of
    GL_ZERO: Result := LongWord(D3DBLEND_ZERO);
    GL_ONE:  Result := LongWord(D3DBLEND_ONE);
  else
    Result := factor - $02FD;
  end;
end;
...
begin
  ...
  d3dDevice.SetTransform( TD3DTransformStateType(d3dMatrixMode), d3dMatrices[ LongWord( d3dMatrixMode ) ] );
  DEC( pushCount );
end;
...
begin
  case mode of
    GL_MODELVIEW:  d3dMatrixMode := LongWord(D3DTS_VIEW);
    GL_PROJECTION: d3dMatrixMode := LongWord(D3DTS_PROJECTION);
    GL_TEXTURE:    d3dMatrixMode := LongWord(D3DTS_TEXTURE0);
  end;
end;
...
begin
  if pname = GL_TEXTURE_MAX_ANISOTROPY_EXT Then
    begin
      {$IFDEF USE_DIRECT3D8}
      d3dDevice.SetTextureStageState( 0, D3DTSS_MAGFILTER, LongWord(D3DTEXF_ANISOTROPIC) );
      d3dDevice.SetTextureStageState( 0, D3DTSS_MINFILTER, LongWord(D3DTEXF_ANISOTROPIC) );
      d3dDevice.SetTextureStageState( 0, D3DTSS_MIPFILTER, LongWord(D3DTEXF_ANISOTROPIC) );
      d3dDevice.SetTextureStageState( 0, D3DTSS_MAXANISOTROPY, oglAnisotropy );
      {$ENDIF}
      {$IFDEF USE_DIRECT3D9}
      ...
      d3dDevice.SetSamplerState( 0, D3DSAMP_MAXANISOTROPY, oglAnisotropy );
      {$ENDIF}
      lMinFilter  := LongWord(D3DTEXF_ANISOTROPIC);
      lMagFilter  := LongWord(D3DTEXF_ANISOTROPIC);
      lMipFilter  := LongWord(D3DTEXF_ANISOTROPIC);
      goto _exit;
    end;
  ...
  case Round( param ) of
    GL_NEAREST: value := LongWord(D3DTEXF_POINT);
    GL_LINEAR: value := LongWord(D3DTEXF_LINEAR);
    {$IFDEF USE_DIRECT3D8}
    GL_LINEAR_MIPMAP_NEAREST: value := LongWord(D3DTEXF_FLATCUBIC);
    GL_LINEAR_MIPMAP_LINEAR:  value := LongWord(D3DTEXF_GAUSSIANCUBIC);
    {$ENDIF}
    {$IFDEF USE_DIRECT3D9}
    // FIXME:
...
begin
  ...
  case param of
    GL_CLAMP_TO_EDGE: value := LongWord(D3DTADDRESS_CLAMP);
    GL_REPEAT: value := LongWord(D3DTADDRESS_WRAP);
  else
...
begin
  ...
  if ( pname = GL_TEXTURE_ENV_MODE ) and ( param = GL_MODULATE ) Then
    begin
      d3dDevice.SetTextureStageState( 0, D3DTSS_COLOROP, LongWord(D3DTOP_MODULATE) );
      d3dDevice.SetTextureStageState( 0, D3DTSS_COLORARG1, D3DTA_TEXTURE );
      d3dDevice.SetTextureStageState( 0, D3DTSS_COLORARG2, D3DTA_DIFFUSE );

      d3dDevice.SetTextureStageState( 0, D3DTSS_ALPHAOP, LongWord(D3DTOP_MODULATE) );
  ...
  case param of
    ...
    GL_MODULATE:          value := LongWord(D3DTOP_MODULATE);
