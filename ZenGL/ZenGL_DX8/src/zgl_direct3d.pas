...
begin
  ...
  for i := 0 to modeCount - 1 do
    begin
      ...
      if ( d3d_GetFormatID( LongWord(d3dMode.Format) ) > d3d_GetFormatID( LongWord(d3dFormat) ) ) Then d3dFormat := d3dMode.Format;
    end;
  {$ENDIF}
  {$IFDEF USE_DIRECT3D9}
...
begin
  ...
  d3dDevice.SetRenderState( D3DRS_CULLMODE, LongWord(D3DCULL_NONE) );
...
begin
  case Format of
    LongWord(D3DFMT_X8R8G8B8): Result := 4;
    LongWord(D3DFMT_A8R8G8B8): Result := 5;
  else
    Result := 0;
  end;
end;
