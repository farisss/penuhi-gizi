...
// CHANGELOGS:
// 1.0.x - Private release
// 1.0.2 - Ported to support FPC, use lstrcmpi to non case-sensitive filename
//         comparsion, and tiny assembly hacks on callback.
...
begin
  ...
  {$IFDEF WIN32API}
  TempHn:= CreateFileW(BundlePath, GENERIC_READ, FILE_SHARE_READ, nil, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
  ...
  {$ELSE}
  {$I-}
  AssignFile(f, BundlePath);
  Reset(f, 1);
  fsz:= FileSize(f);
  if fsz >= SizeOf(TBundleHeader) then
    begin
    BlockRead(f, BunHdr, SizeOf(BunHdr));
    if BunHdr.BundleId = BundHeaderString then
      if BunHdr.FileOnBundle > 0 then
        begin
        SizeMemBun:= SizeOf(TBundleDefinition)+(BunHdr.FileOnBundle*SizeOf(TBundleFile));
        GetMem(ptr, SizeMemBun);
        FillChar(ptr^, SizeMemBun, 0);
        ptr.BundleHandle:= f;
        ptr.FileOnBundle:= BunHdr.FileOnBundle;
        ptr.SizeOfMem:= SizeMemBun;
        copyPtr:= Pointer(Cardinal(ptr)+SizeOf(TBundleDefinition));
        for x:= 0 to BunHdr.FileOnBundle-1 do
          begin
          BlockRead(f, copyPtr^, SizeOf(TBundleFile));
          copyPtr:= Pointer(Cardinal(copyPtr)+SizeOf(TBundleFile));
        end;
        Result:= ptr;
        Exit;
...
begin
  ...
  {$ELSE}
  Move(FilePath, BundleSrc^.FilePath, flen);
  {$ENDIF}
  ...
  if AliasName <> nil then
    ...
    {$ELSE}
    Move(AliasName, BundleSrc^.AliasName, alen);
    {$ENDIF}
...
begin
  ...
  while (x < Bundle.FileOnBundle) and (not FileFound) do
    ...
    {$IFDEF WIN32API}
    if lstrcmpi(PChar(DestName), PChar(name)) = 0 then
    {$ELSE}
    DestName:= UpperCase(DestName);
    name:= UpperCase(name);
...
begin
  ...
  {$ELSE}
  Reset(Bundle.BundleHandle, RealPos);
  {$ENDIF}
...
begin
  ...
  {$ELSE}
  try
    BlockRead(Bundle^.BundleHandle, Buf, Size);
    Result:= TRUE;
  except
    Result:= FALSE;
  end;
